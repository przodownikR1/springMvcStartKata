...
public class TagJdbcRepositoryImpl implements TagJdbcRepository {
    ...
    public Tag findTagByName(String name) {
        final List<Tag> tags = this.jdbcTemplate.query("SELECT id, name, date_added , date_modyfication  FROM Tag WHERE name=?",
                new ParameterizedRowMapper<Tag>() {
                    @Override
        ...
        if (!tags.isEmpty()) { return tags.get(0); }
        return null;
    }

    @Override
    public Tag getTagById(long id) {
        final List<Tag> tags = this.jdbcTemplate.query("SELECT id, name, date_added , date_modyfication  FROM Tag WHERE id=?",
                new ParameterizedRowMapper<Tag>() {
                    @Override
                    public Tag mapRow(ResultSet rs, int row) throws SQLException {
                        Tag tag = new Tag();
    ...
    public List<Tag> getAllTag() {
        final List<Tag> tags = this.jdbcTemplate.query("SELECT id, name, date_added , date_modyfication  FROM Tag ", new ParameterizedRowMapper<Tag>() {
            @Override
            public Tag mapRow(ResultSet rs, int row) throws SQLException {
                Tag tag = new Tag();
                tag.setId(rs.getLong("id"));
